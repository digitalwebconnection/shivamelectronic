...
export const SmartphonesSection: React.FC<SmartphonesSectionProps> = ({
  ...
  return (
    <section 
      id="category-cables" 
      className="pt-10 py-0 md:py-10 bg-linear-to-b from-white via-rose-50/20 to-white border-b border-slate-100 scroll-mt-20 relative overflow-hidden"
    >
      {/* Background soft glowing highlights */}
      <div className="absolute top-1/3 right-0 w-87.5 h-87.5 bg-rose-200/10 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-10 left-10 w-62.5 h-62.5 bg-amber-200/5 rounded-full blur-[80px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-6 relative z-10">
        ...
        <div className="flex overflow-x-auto gap-4 pb-4 snap-x hide-scrollbar lg:pb-6 px-4 sm:px-0 -mx-4 sm:mx-0">
          ...
          <div className="rounded-md bg-linear-to-b from-red-800 via-red-500/80 to-rose-100 p-6 sm:p-7 flex flex-col text-white relative overflow-hidden shadow-xl shadow-red-600/20 hover:shadow-2xl hover:shadow-red-600/30 transition-all duration-500 group shrink-0 snap-start w-70 sm:w-75 md:w-75 lg:w-80">
            {/* Mesh Background */}
            ...
            <button 
              onClick={() => onViewAll('cables')}
              className="mt-6 w-full py-3.5 px-4 bg-white hover:bg-slate-50 text-red-600 font-black text-[10px] tracking-widest rounded-lg flex items-center justify-center gap-2 active:scale-95 transition-all shadow-xl shadow-red-900/10 cursor-pointer relative z-10"
            >
          ...
          {cables.slice(0, 10).map(p => {
            ...
            return (
              <div 
                ...
                className="group relative flex flex-col bg-white border border-slate-100 rounded-md aspect-4/5 overflow-hidden shadow-xl shadow-slate-800/45 hover:shadow-2xl hover:shadow-red-550/15 hover:-translate-y-1.5 transition-all duration-500 cursor-pointer shrink-0 snap-start w-55 sm:w-60 md:w-60 lg:w-65"
              >
                {/* Wishlist Button */}
                <button
                  onClick={(e) => {
