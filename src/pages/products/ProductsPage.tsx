...
export const ProductsPage: React.FC<ProductsPageProps> = ({
  ...
  // Compute maximum price dynamically from data
  const maxPriceLimit = useMemo(() => {
    if (products.length === 0) return 20;
    return Math.ceil(Math.max(...products.map(p => p.price)));
  }, [products]);

  // Sync price range limit on mount or products load
  useEffect(() => {
  ...
  const filteredProducts = useMemo(() => {
    return products
      .filter(product => {
        ...
        const matchesHot = !showOnlyHot || product.isHot;

        return matchesCategory && matchesSearch && matchesBrand && matchesPrice && matchesNew && matchesHot;
      })
      .sort((a, b) => {
  ...
  }, [products, selectedCategory, searchQuery, selectedBrands, priceRange, showOnlyNew, showOnlyHot, sortBy]);
  ...
  const isFiltered = useMemo(() => {
    return selectedCategory !== 'All' || 
      ...
      sortBy !== 'default' ||
      categorySearch.trim() !== '';
  }, [selectedCategory, searchQuery, selectedBrands, priceRange, maxPriceLimit, showOnlyNew, showOnlyHot, sortBy, categorySearch]);
    ...
    <div className="space-y-7">
      ...
      <div className="space-y-3.5 pt-4 border-t border-slate-100">
        ...
        <div className="px-1">
          ...
          />
          <div className="flex items-center justify-between text-xs font-bold text-slate-500 mt-3">
            <span>$0.00</span>
            <span className="text-blue-600 bg-blue-50/70 border border-blue-100/50 px-2 py-0.5 rounded-md font-extrabold text-[10px]">
      ...
      {/* Status Badges */}
      <div className="space-y-3 pt-4 border-t border-slate-100">
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Special Offers</h3>
        <div className="flex flex-col gap-2.5">
          <label className="flex items-center gap-2.5 cursor-pointer text-xs font-semibold text-slate-700 hover:text-slate-900 transition-colors">
            <input 
              type="checkbox"
              checked={showOnlyNew}
          ...
          <label className="flex items-center gap-2.5 cursor-pointer text-xs font-semibold text-slate-700 hover:text-slate-900 transition-colors">
            <input 
              type="checkbox"
              checked={showOnlyHot}
              onChange={() => setShowOnlyHot(!showOnlyHot)}
              className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 w-4 h-4 cursor-pointer"
            />
            <div className="flex items-center gap-1">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center space-y-3 mb-10">
          ...
          <p className="text-xs sm:text-sm text-slate-500 max-w-xl mx-auto">
            Browse our verified inventory of high-grade electronic components. Use the sidebar filters to refine by Category, Brand, Price range and stock options.
          </p>
        </div>
            ...
            {filteredProducts.length > 0 ? (
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between">
                        ...
                        <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100">
                          <div className="flex flex-col">
                            <span className="text-[8px] text-slate-400 uppercase font-black tracking-wider leading-none mb-0.5">Price</span>
              ...
              <div className="text-center py-20 bg-white border border-slate-200 rounded-3xl p-8 max-w-md mx-auto shadow-sm animate-in fade-in duration-300">
                ...
                <p className="text-xs text-slate-500 leading-relaxed mb-6">
                  We couldn't find any products matching your current combination of categories, search queries, brands, or price limits.
                </p>
                {isFiltered && (
                  <button
                    onClick={handleClearFilters}
